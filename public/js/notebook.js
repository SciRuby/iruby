// This is derived from IRuby's Notebook.js, which is BSD licensed.
//============================================================================
// Notebook
//============================================================================
...
var IRuby = (function (IRuby) {
  ...
  Notebook.prototype.select = function (index) {
    if (index !== undefined && index >= 0 && index < this.ncells()) {
      ...
      if (cell.cell_type === 'heading') {
        $([IRuby.events]).trigger('selected_cell_type_changed.Notebook',
  ...
  Notebook.prototype.insert_cell_below = function (type, index) {
    ...
    if (this.ncells() === 0 || this.is_valid_cell_index(index)) {
      if (type === 'code') {
        cell = new IRuby.CodeCell(this);
        cell.set_input_prompt();
      } else if (type === 'markdown') {
        cell = new IRuby.MarkdownCell(this);
      } else if (type === 'html') {
        cell = new IRuby.HTMLCell(this);
      } else if (type === 'plaintext') {
        cell = new IRuby.PlaintextCell(this);
      } else if (type === 'heading') {
        cell = new IRuby.HeadingCell(this);
      };
      if (cell !== null) {
        if (this.ncells() === 0) {
  ...
  Notebook.prototype.to_code = function (index) {
    ...
    if (this.is_valid_cell_index(i)) {
      ...
      if (!(source_cell instanceof IRuby.CodeCell)) {
  ...
  Notebook.prototype.to_markdown = function (index) {
    ...
    if (this.is_valid_cell_index(i)) {
      var source_element = this.get_cell_element(i);
      var source_cell = source_element.data("cell");
      if (!(source_cell instanceof IRuby.MarkdownCell)) {
  ...
  Notebook.prototype.to_html = function (index) {
    ...
    if (this.is_valid_cell_index(i)) {
      ...
      if (!(source_cell instanceof IRuby.HTMLCell)) {
  ...
  Notebook.prototype.to_plaintext = function (index) {
    ...
    if (this.is_valid_cell_index(i)) {
      ...
      var source_cell = source_element.data("cell");
      var target_cell = null;
      if (!(source_cell instanceof IRuby.PlaintextCell)) {
  ...
  Notebook.prototype.to_heading = function (index, level) {
    ...
    if (this.is_valid_cell_index(i)) {
      ...
      var source_cell = source_element.data("cell");
      var target_cell = null;
      if (source_cell instanceof IRuby.HeadingCell) {
  ...
  Notebook.prototype.split_cell = function () {
    ...
    if (cell.is_splittable()) {
      ...
      if (cell instanceof IRuby.CodeCell) {
        cell.set_text(texta);
        var new_cell = this.insert_cell_below('code');
        new_cell.set_text(textb);
      } else if (cell instanceof IRuby.MarkdownCell) {
        cell.set_text(texta);
        cell.render();
        ...
        new_cell.set_text(textb);
        new_cell.render();
      } else if (cell instanceof IRuby.HTMLCell) {
        cell.set_text(texta);
        cell.render();
  ...
  Notebook.prototype.merge_cell_above = function () {
    ...
    if (index > 0) {
      ...
      } else if (cell instanceof IRuby.MarkdownCell || cell instanceof IRuby.HTMLCell) {
        cell.edit();
        cell.set_text(upper_text+'\n'+text);
        cell.render();
      };
  ...
  Notebook.prototype.clear_all_output = function () {
    ...
    for (var i=0; i<ncells; i++) {
      if (cells[i] instanceof IRuby.CodeCell) {
  ...
  Notebook.prototype.handle_payload = function (cell, payload) {
    ...
    for (var i=0; i<l; i++) {
      if (payload[i].source === 'IRuby.zmq.page.page') {
        if (payload[i].text.trim() !== '') {
          IRuby.pager.clear();
          IRuby.pager.expand();
          IRuby.pager.append_text(payload[i].text);
        }
      } else if (payload[i].source === 'IRuby.zmq.zmqshell.ZMQInteractiveShell.set_next_input') {
        var index = this.find_cell_index(cell);
  ...
  Notebook.prototype.handle_iopub_reply = function (e) {
    ...
    if (output_types.indexOf(msg_type) >= 0) {
      ...
      if (content.execution_state === 'busy') {
        $([IRuby.events]).trigger('status_busy.Kernel');
      } else if (content.execution_state === 'idle') {
        $([IRuby.events]).trigger('status_idle.Kernel');
  ...
  Notebook.prototype.save_notebook = function () {
    ...
    $([IRuby.events]).trigger('notebook_saving.Notebook');
  ...
  Notebook.prototype.save_notebook_success = function (data, status, xhr) {
    this.dirty = false;
    $([IRuby.events]).trigger('notebook_saved.Notebook');
  };


  Notebook.prototype.save_notebook_error = function (xhr, status, error_msg) {
    $([IRuby.events]).trigger('notebook_save_failed.Notebook');
  };
  ...
  Notebook.prototype.load_notebook = function (notebook_id) {
    ...
    $([IRuby.events]).trigger('notebook_loading.Notebook');
    var url = $('body').data('baseProjectUrl') + 'notebooks/' + this.notebook_id;
    //$.ajax(url, settings); // We don't support saving or loading notebooks yet
    var fake_notebook = {
      metadata: {
        name: "Untitled0"
      },
    ...
    this.load_notebook_success(fake_notebook);
  };
  ...
  Notebook.prototype.load_notebook_success = function (data, status, xhr) {
    ...
    if (data.orig_nbformat !== undefined && data.nbformat !== data.orig_nbformat) {
      msg = "This notebook has been converted from an older " +
        ...
        "newer notebook format will be used and older verions of IRuby " +
    ...
    $([IRuby.events]).trigger('notebook_loaded.Notebook');
  };
  ...
  Notebook.prototype.load_notebook_error = function (xhr, textStatus, errorThrow) {
    if (xhr.status === 500) {
      msg = "An error occurred while loading this notebook. Most likely " +
        ...
        "version of IRuby. This version can load notebook formats " +
