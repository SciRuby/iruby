...
var IRuby = (function (IRuby) {
  ...
  var Kernel = function () {
    ...
    this.base_url = $('body').data('baseKernelUrl');
    this.running = false;
  ...
  Kernel.prototype.restart = function (callback) {
    $([IRuby.events]).trigger('status_restarting.Kernel');
  ...
  Kernel.prototype.start_channels = function () {
    ...
    this.shell_channel = new this.WebSocket(ws_url);
  ...
  Kernel.prototype.interrupt = function () {
    if (this.running) {
      $([IRuby.events]).trigger('status_interrupting.Kernel');
